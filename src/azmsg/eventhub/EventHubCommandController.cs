...
namespace azmsg.eventhub
{
    class EventHubCommandController : ICommandController
    {
        ...
        public Command CreateCommand()
        {
            ...
            // get-context
            var eventHubGetContextCommand = new Command("get-context");
            eventHubGetContextCommand.Add(new Option<string>("--name"));
            eventHubGetContextCommand.Handler = CommandHandler.Create<string>(GetContext);
            eventhubCommand.AddCommand(eventHubGetContextCommand);
        ...
        public void GetContext(string name)
        {
            ...
            {
                foreach (var kv in config.EventHubContexts)
                {
            ...
            {
                if (config.EventHubContexts.ContainsKey(name))
                {
                    service.UpdateConfig(config);
                    Console.WriteLine(JsonSerializer.Serialize(config.EventHubContexts[name], options));
                }
                else
                {
